    ...
    <section className="relative bg-[#151412] text-[#F7F3EC] pt-28 sm:pt-32 pb-20 lg:pb-28 border-b border-[#C6A66B]/20 overflow-hidden">
      ...
      {bgImage && (
        <div className="absolute inset-0 z-0">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-[#151412] via-[#151412]/80 to-[#151412]/90"></div>
        </div>
      )}

      {/* Decorative ambient radial glow */}
      <div className="absolute top-0 right-1/4 w-96 h-96 bg-[#C6A66B]/5 rounded-full filter blur-[140px] pointer-events-none"></div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
        ...
        {/* Main Headline */}
        <h1 className="font-serif text-4xl sm:text-6xl lg:text-7xl uppercase tracking-tight text-[#F7F3EC] font-light leading-[1.0] max-w-4xl mb-6">
          {title} {italicTitle && <span className="italic text-gold-gradient font-normal">{italicTitle}</span>}
        </h1>

        {/* Supporting description */}
        {description && (
          <p className="font-sans text-sm sm:text-base md:text-lg text-[#E8DED0]/85 max-w-2xl font-light leading-relaxed tracking-wide">
            {description}
          </p>
        )}
      </div>
